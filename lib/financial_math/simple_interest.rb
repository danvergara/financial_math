module FinancialMath
  module SimpleInterest
    @discount = ->(present_value, future_value) { future_value - present_value }
    ...
    def interest_rate(discount_rate:, periods: 0, frequency: 1)
      divisor = 1 - discount_rate * factor(periods: periods, frequency: frequency)
      (discount_rate / divisor).round(2)
    end

    def discount_rate(interest_rate:, periods: 0, frequency: 1)
      divisor = 1 + interest_rate * factor(periods: periods, frequency: frequency)
      (interest_rate / divisor).round(2)
    end

    def rational_discount(present_value:, interest_rate:, periods: 0, frequency: 1)
      factor_val = factor(periods: periods, frequency: frequency)
